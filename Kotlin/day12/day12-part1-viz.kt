            ...
            section {
                grid.forEachIndexed { rowIndex, row ->
                    row.forEachIndexed { columnIndex, column ->
                        if ((rowIndex to columnIndex) == start) {
                            hsv(118, 1f, .30f) {
                                text('a')
                            }
                            ...
                            hsv(210, 1f, 0.68f * (1f/max(tileAges[(rowIndex * grid[0].size) + columnIndex], 1)) + 0.32f) {
                                text(column)
                            }
                        } else {
                            hsv(0, 0f, .5f) {
